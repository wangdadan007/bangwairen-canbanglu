...
try {
  for (const viewport of VIEWPORTS) {
    ...
    await captureBattleLayoutFlow(page, viewport.label)
...
async function captureBattleLayoutFlow(page, viewportLabel) {
  ...
  await page.locator('.battle-screen-shell').waitFor()
  ...
  await capture(page, viewportLabel, reachedMultiEnemy ? '04-multi-enemy-layout' : '04-multi-enemy-fallback', {
    focus: page.locator('.battle-screen-shell'),
    note: reachedMultiEnemy ? 'multi-enemy battle reached' : 'multi-enemy not reached before boss path',
  })
...
async function capture(page, viewportLabel, label, options = {}) {
  ...
  const metrics = await page.evaluate(() => {
    ...
    return {
      battleCaseAxisVisible: Boolean(document.querySelector('.battle-case-axis')),
      battleCommandZoneVisible: Boolean(document.querySelector('.battle-command-zone')),
      ...
      logDrawerVisible: Boolean(document.querySelector('.battle-log-drawer')),
...
async function continueUntilMultiEnemyBattle(page) {
  for (let step = 0; step < 80; step += 1) {
    if (await isVisible(page.locator('.battle-screen-shell'))) {
      const enemyCount = await page.locator('.enemy-panel').count()
      const bossVisible = await isVisible(page.getByText('Boss：窃榜使'))
      if (enemyCount > 1) {
        return true
      }
      if (bossVisible) {
    ...
    if (await advanceNonBattlePage(page)) {
      continue
    }
...
async function advanceNonBattlePage(page) {
